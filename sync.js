...
(function(){
  ...
  function mergeRemote(r){
    ...
    for(const x of r.workouts||[]){const local=by.get(x.id);if(!local||Number(x.updatedAt||0)>Number(local.updatedAt||0)){if(x.deleted)removeLocalWorkout(x.id);else{if(!db.logs[x.date])db.logs[x.date]={};if(!db.logs[x.date][x.exercise])db.logs[x.date][x.exercise]=[];for(const d of Object.keys(db.logs))for(const [k] of EX)db.logs[d][k]=db.logs[d][k].filter(e=>e.id!==x.id);db.logs[x.date][x.exercise].push({...x});}}}
